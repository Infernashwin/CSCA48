...
int ingredient_index(char source_ingredient[MAX_STR_LEN])
{
     ...
     *****/
    for(int i = 0; i<MAT_SIZE; i++) if (!strcmp(ingredients[i],source_ingredient)) return i;
    return -1;
}
...
void related_ingredients(char source_ingredient[MAX_STR_LEN])
{
    ...
    for(int j=0; j<MAT_SIZE; j++) if (AdjMat[index][j]>0 && index > -1) printf("%s\n", ingredients[j]);
}
...
intNode *related_k_dist(intNode *h, char source_ingredient[MAX_STR_LEN], int k, int dist)
{
    ...
    if (dist >= k) return h;
    int index = ingredient_index(source_ingredient);
    for (int j=0; j<MAT_SIZE && index > -1; j++) if (AdjMat[index][j]>0){
      if(!searchInt(h, j)) h = insertInt(h, j);
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
    ...
    intNode *related = NULL;
    intNode *final = NULL;
    intNode *p = NULL;
    restricted = insertInt(restricted, ingredient_index(avoid));
    restricted = related_k_dist(restricted, avoid, k_avoid, 0);
    related = related_k_dist(related, source_ingredient, k_source, 0);
    p = related;
    while (p){
      if(!(searchInt(restricted, p->x) || searchInt(final, p->x))) final = insertInt(final, p->x);
      p = p->next;
    }
    deleteList(restricted);
    deleteList(related);
    return final;
}
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
   ...
   */
   ...
   int max = -1;
   int index = -1;
   int pos_ingredients[MAT_SIZE] = {0};
   int food_index;
   intNode *recipe_list = NULL;

  for(int i = 0; i<10; i++){
    if (strcmp(recipe[i], (""))){
      recipe_list = insertInt(recipe_list, ingredient_index(recipe[i]));
      if (!strcmp(recipe[i], to_change)) index = i;
      else{
      food_index = ingredient_index(recipe[i]);
      for(int j = 0; j<MAT_SIZE; j++) pos_ingredients[j] += AdjMat[food_index][j];
      }
    }
  }

  if (index < 0){
    deleteList(recipe_list);
    return;
  }

  for(int i = 0; i<MAT_SIZE; i++) if (pos_ingredients[i] > max && !searchInt(recipe_list, i)){
    max = pos_ingredients[i];
    strcpy(recipe[index], ingredients[i]);
  }

  if (!strcmp(recipe[index], to_change)) strcpy(recipe[index], "");
