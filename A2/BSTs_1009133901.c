...
BST_Node *BST_search(BST_Node *root, int bar, double index)
{
    ...
    if (root->key == (bar*10 + index)) return root;

    else if (root->key > bar*10+index) return BST_search(root->left, bar, index);

    else return BST_search(root->right, bar, index);
}
...
BST_Node *BST_delete(BST_Node *root, int bar, double index)
{
    ...
    BST_Node *node = NULL;
    if (!root) return NULL;
    if (root->key > bar*10+index) root->left = BST_delete(root->left, bar, index);
    else if (root->key < bar*10+index) root->right = BST_delete(root->right, bar, index);
    else{
        if (!(root->left || root->right)){
            ...
            root->key = node->key;
            root->right = BST_delete(root->right, node->bar, node->index);
            return root;
        }
    }
    return root;
...
void BST_makePlayList(BST_Node *root)
{
     ...
     ****/
    if (!root) return;
    if (root->left) BST_makePlayList(root->left);
    playlist_head = playlist_insert(playlist_head, root->freq, root->bar, root->index);
    if (root->right) BST_makePlayList(root->right);
...
void BST_inOrder(BST_Node *root, int depth)
{
     ...
     ****/
    if (!root) return;
    if (root->left) BST_inOrder(root->left, depth+1);
    printf("Depth=%d, Bar:Index (%d,%f), F=%f Hz\n", depth, root->bar, root->index, root->freq);
    if (root->right) BST_inOrder(root->right, depth+1);
...
void BST_preOrder(BST_Node *root, int depth)
{
    ...
    if (!root) return;
    printf("Depth=%d, Bar:Index (%d,%f), F=%f Hz\n", depth, root->bar, root->index, root->freq);
    if (root->left) BST_preOrder(root->left, depth+1);
    if (root->right) BST_preOrder(root->right, depth+1);
...
void BST_postOrder(BST_Node *root,int depth)
{
     ...
     ****/
    if (!root) return;
    if (root->left) BST_postOrder(root->left, depth+1);
    if (root->right) BST_postOrder(root->right, depth+1);
    printf("Depth=%d, Bar:Index (%d,%f), F=%f Hz\n", depth, root->bar, root->index, root->freq);
}
...
void delete_BST(BST_Node *root)
{
     ...
     ****/
    if (!root) return;
    if (root->left) delete_BST(root->left);
    if (root->right) delete_BST(root->right);
    free(root);
...
BST_Node *reverse_nodes(BST_Node *root, int high, int low){
    if (!root) return NULL;
    BST_Node *node = root->right;
    root->bar = high - root->bar + low;
    root->index = 1 - root->index;
    root->key = 10*root->bar + root->index;
    root->right = reverse_nodes(root->left, high, low);
    root->left = reverse_nodes(node, high, low);
    return root;
}
...
BST_Node *reverseSong(BST_Node *root)
{
     ...
     ****/
    int high = find_max(root);
    int low = find_min(root);
    root = reverse_nodes(root, high, low);
    return root;
 ...
 * ******************************************************************************************/
void insert_harmonies(BST_Node *root, BST_Node *shiftee, int semitones, double time_shift){
    double freq = shiftee->freq;
    double index = -1.0;
    int i = 0;
    if (!shiftee) return;
    if (shiftee->left) insert_harmonies(root, shiftee->left, semitones, time_shift);
    if (shiftee->right) insert_harmonies(root, shiftee->right, semitones, time_shift);
    for (i = 0; i<=99; i++){
        if (note_freq[i] == shiftee->freq && 0<=semitones+i && semitones+i<=99){
            freq = note_freq[i+semitones];
            index = shiftee->index + time_shift;
            break;
        }
    }
    if(0<=index && index<1) BST_insert(root, newBST_Node(freq, shiftee->bar, index));
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
     ...
     ****/
    if (!root) return NULL;
    insert_harmonies(root, root, semitones, time_shift);
    return root;
